...
struct GroqWhisperTranscriptionProviderError: LocalizedError {
    let message: String

    var errorDescription: String? { message }
}

final class GroqWhisperTranscriptionProvider: SpeechToTextProvider {
    private let apiKey = AppBundleConfiguration.stringValue(forKey: "GroqAPIKey")
    private let modelName = AppBundleConfiguration.stringValue(forKey: "GroqTranscriptionModel")
        ?? "whisper-large-v3-turbo"

    let displayName = "Groq Whisper Turbo"
    ...
    var unavailableExplanation: String? {
        ...
        return "Groq transcription is not configured. Add GroqAPIKey to Info.plist."
    }
    ...
    func startStreamingSession(
        ...
        guard let apiKey else {
            throw GroqWhisperTranscriptionProviderError(
                message: unavailableExplanation ?? "Groq transcription is not configured."
            )
        }
...
private final class GroqWhisperTranscriptionSession: SpeechToTextStreamingSession {
    ...
    private let stateQueue = DispatchQueue(label: "com.orbit.groq.transcription")
    ...
    private func requestTranscription(for wavAudioData: Data) async throws -> String {
        ...
        guard (200...299).contains(httpResponse.statusCode) else {
            ...
            throw GroqWhisperTranscriptionProviderError(
                message: "Groq transcription failed: \(responseText)"
            )
        }
        ...
        throw GroqWhisperTranscriptionProviderError(message: "Groq transcription returned an empty transcript.")
    }
