//
//  OrbitDictationManager.swift
//  Orbit
//
//  Shared push-to-talk dictation manager for Orbit voice input.
...
enum OrbitDictationPermissionProblem {
    case microphoneAccessDenied
    case speechRecognitionDenied
}

private enum OrbitDictationStartSource {
    case microphoneButton
    case keyboardShortcut
}

private struct OrbitDictationDraftCallbacks {
    let updateDraftText: (String) -> Void
    let submitDraftText: (String) -> Void
}

@MainActor
final class OrbitDictationManager: NSObject, ObservableObject {
    ...
    @Published private(set) var recordedAudioPowerHistory = Array(
        repeating: OrbitDictationManager.recordedAudioPowerHistoryBaselineLevel,
        count: OrbitDictationManager.recordedAudioPowerHistoryLength
    )
    ...
    @Published private(set) var currentPermissionProblem: OrbitDictationPermissionProblem?
    ...
    private var activeStartSource: OrbitDictationStartSource?
    private var draftCallbacks: OrbitDictationDraftCallbacks?
    ...
    override init() {
        let transcriptionProvider = OrbitSpeechToTextProviderFactory.makeDefaultProvider()
        self.transcriptionProvider = transcriptionProvider
        self.transcriptionProviderDisplayName = transcriptionProvider.displayName
        super.init()
        settingsCancellable = OrbitSettings.shared.$voicePreset
    ...
    func refreshConfiguredProviders() {
        ...
        updateTranscriptionProvider(for: OrbitSettings.shared.voicePreset)
    }
    ...
    private func startPushToTalk(
        startSource: OrbitDictationStartSource,
        currentDraftText: String,
        updateDraftText: @escaping (String) -> Void,
        submitDraftText: @escaping (String) -> Void,
        ...
        print("🎙️ OrbitDictationManager: start requested (\(startSource))")

        if needsInitialPermissionPrompt {
            print("🎙️ OrbitDictationManager: requesting initial permissions")
            NSApplication.shared.activate(ignoringOtherApps: true)
        ...
        guard await requestMicrophoneAndSpeechPermissionsWithoutDuplicatePrompts() else {
            print("🎙️ OrbitDictationManager: permissions missing or denied")
            isPreparingToRecord = false
            return
        }
        guard !Task.isCancelled else {
            print("🎙️ OrbitDictationManager: start cancelled (shortcut released during permission check)")
            isPreparingToRecord = false
            return
        }
        guard pendingStartRequestIdentifier == startRequestIdentifier else {
            print("🎙️ OrbitDictationManager: start request superseded")
            isPreparingToRecord = false
            return
        }
        ...
        draftCallbacks = OrbitDictationDraftCallbacks(
            updateDraftText: updateDraftText,
            submitDraftText: submitDraftText
        )
        ...
        guard !Task.isCancelled else {
            print("🎙️ OrbitDictationManager: start cancelled (shortcut released before recording began)")
            resetSessionState()
            return
        }
        ...
        do {
            ...
            guard !Task.isCancelled else {
                print("🎙️ OrbitDictationManager: start cancelled (shortcut released during session start)")
                audioEngine.stop()
                audioEngine.inputNode.removeTap(onBus: 0)
                activeTranscriptionSession?.cancel()
                resetSessionState()
            ...
            isPreparingToRecord = false
            print("🎙️ OrbitDictationManager: recognition session started")
        } catch {
            isPreparingToRecord = false
            ...
            print("❌ OrbitDictationManager: failed to start recognition session (\(transcriptionProvider.displayName)): \(error)")
            resetSessionState()
        }
    }

    private func stopPushToTalk(expectedStartSource: OrbitDictationStartSource) {
        pendingStartRequestIdentifier = UUID()
        ...
        print("🎙️ OrbitDictationManager: stop requested (\(expectedStartSource))")
    ...
    private func startRecognitionSession() async throws {
        ...
        print("🎙️ OrbitDictationManager: opening transcription provider \(transcriptionProvider.displayName)")
    ...
    private func handleRecognitionError(_ error: Error) {
        ...
        if isFinalizingTranscript && !latestRecognizedText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            ...
            print("❌ Orbit dictation error (\(transcriptionProvider.displayName)): \(error)")
            lastErrorMessage = userFacingErrorMessage(
                from: error,
    ...
    private func updateTranscriptionProvider(for voicePreset: OrbitVoicePreset) {
        guard !isDictationInProgress else { return }
        let updatedProvider = OrbitSpeechToTextProviderFactory.makeProvider(for: voicePreset)
